...
import { formatDayMonthYear, getCategoriesWithHabits, getLogsForDate } from "../db/db-repo";
...
export default function ChicagoHeaderTitle() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const totalHabits = allCategories.reduce((sum, c) => sum + c.habits.length, 0);
  ...
  return (
    <View>
      <View style={styles.row}>
        ...
        <Text style={[styles.sep, { color: theme.textSecondary }]}> · </Text>
        <Text style={[styles.stat, { color: theme.textPrimary }]}>{gymLeft}</Text>
        <Text style={[styles.unit, { color: theme.textSecondary }]}> gym left</Text>
      </View>
      {habitsLeft !== null && (
        <Text style={[styles.habits, { color: allDone ? "#22C55E" : theme.textSecondary }]}>
          {allDone
